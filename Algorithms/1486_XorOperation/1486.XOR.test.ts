import { xorOperation } from './1486.XOR_Operation';
...
describe('XOR Operations in an array', () => {
	...
	it('Input: n = 5, start = 0', () => {
		expect(xorOperation(5, 0)).to.eqls(8);
	});
	it('Input: n = 4, start = 3', () => {
		expect(xorOperation(4, 3)).to.eqls(8);
	});
	it('Input: n = 1, start = 7', () => {
		expect(xorOperation(1, 7)).to.eqls(7);
	});
	it('Input: n = 10, start = 5', () => {
		expect(xorOperation(10, 5)).to.eqls(2);
	});
});
